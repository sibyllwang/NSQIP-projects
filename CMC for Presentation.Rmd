...
ASAHIGH<-function(df){
  ASAHIGH<-vector()
  for(i in 1:nrow(df)){
    if(df$ASACLAS[i]=="1-No Disturb"
       |df$ASACLAS[i]=="2-Mild Disturb"){
      ASAHIGH[i]<-"No"
    }else{
      ASAHIGH[i]<-"Yes"
    }
...
CMCaddComplication<-ASAHIGH(CMCaddComplication)
```
...
CMCaddComplication<-INSULIN(CMCaddComplication)
```


```{r,echo=FALSE}
CMCaddComplication$AGE<-as.numeric(CMCaddComplication$AGE)
fit2<-glm(Complication~factor(INSULIN)+factor(ASAHIGH)+factor(DIALYSIS)+AGE+PRHCT, data = CMCaddComplication, family = poisson)
summary(fit2)
```


Odds ratio and confindence interval
```{r, echo = FALSE}
...
```{r}
fit3<-glm(factor(FNSTATUS1)+CPT+factor(HYPERMED)+factor(BLEEDDIS)+factor(DIABETES)+factor(ASACLAS)+factor(DIALYSIS)+AGE+PRHCT+factor(HXCHF)+factor(STEROID)+OPTIME~Complication, data = DRfxaddComplication, family = binomial))
summary(fit3)
```
...
CMCaddComplication<-diatemp(CMCaddComplication)


renatemp<-function(df){
  renatemp<-vector()
  for(i in 1:nrow(df)){
    if(df$RENAFAIL[i]=="Yes"){
      renatemp[i]<-2
    }else{
      renatemp[i]<-0
    }
  }
  df<-cbind(df, renatemp)
}
CMCaddComplication<-renatemp(CMCaddComplication)


hxchftemp<-function(df){
  hxchftemp<-vector()
  for(i in 1:nrow(df)){
    if(df$HXCHF[i]=="Yes"){
      hxchftemp[i]<-1
  ...
  df<-cbind(df, hxchftemp)
}
CMCaddComplication<-hxchftemp(CMCaddComplication)


hxcopdtemp<-function(df){
  hxcopdtemp<-vector()
  for(i in 1:nrow(df)){
    if(df$HXCOPD[i]=="Yes"){
      hxcopdtemp[i]<-1
  ...
  df<-cbind(df, hxcopdtemp)
}
CMCaddComplication<-hxcopdtemp(CMCaddComplication)


hxmitemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, hxmitemp)
filter(CMCaddComplication, HXMI=="Yes")$CASEID
CMCaddComplication[CMCaddComplication$CASEID=="2022109",]$hxmitemp<-1

hxpvdtemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, hxpvdtemp)
filter(CMCaddComplication, HXPVD=="Yes")$CASEID
CMCaddComplication[CMCaddComplication$CASEID=="1157091",]$hxpvdtemp<-1

maltemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, maltemp)
filter(CMCaddComplication, CHEMO=="Yes")$CASEID
filter(CMCaddComplication, RADIO=="Yes")$CASEID
CMCaddComplication[CMCaddComplication$CASEID=="1749129",]$maltemp<-2

cvatemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, cvatemp)
temp<-filter(CMCaddComplication, CVA=="Yes")$CASEID
CMCaddComplication[CMCaddComplication$CASEID %in% temp,]$cvatemp<-2


### DONE
cdmitemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, cdmitemp)
temp<-filter(CMCaddComplication, is.na(CDMI)==FALSE & CDMI=="Myocardial Infarction")$CASEID
CMCaddComplication[CMCaddComplication$CASEID %in% temp,]$cdmitemp<-1

### DONE
hxchftemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, hxchftemp)
temp<-filter(CMCaddComplication, is.na(HXCHF)==FALSE & HXCHF=="Yes")$CASEID
CMCaddComplication[CMCaddComplication$CASEID %in% temp,]$hxchftemp<-1

### DONE
hxpvdtemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, hxpvdtemp)
temp<-filter(CMCaddComplication, HXPVD=="Yes")$CASEID
CMCaddComplication[CMCaddComplication$CASEID %in% temp,]$hxpvdtemp<-1

### DONE
cnscvatemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, cnscvatemp)
temp<-filter(CMCaddComplication, CNSCVA=="Stroke/CVA")$CASEID
CMCaddComplication[CMCaddComplication$CASEID %in% temp,]$cnscvatemp<-1

### DONE
hxcopdtemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, hxcopdtemp)
temp<-filter(CMCaddComplication, HXCOPD=="Yes")$CASEID
CMCaddComplication[CMCaddComplication$CASEID %in% temp,]$hxcopdtemp<-1

### DONE
diabetestemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, diabetestemp)
temp<-filter(CMCaddComplication, DIABETES=="INSULIN"|DIABETES=="ORAL"|DIABETES=="NON-INSULIN")$CASEID
CMCaddComplication[CMCaddComplication$CASEID %in% temp,]$diabetestemp<-1

### DONE
renafailtemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, renafailtemp)
temp<-filter(CMCaddComplication, RENAFAIL=="Yes")$CASEID
CMCaddComplication[CMCaddComplication$CASEID %in% temp,]$renafailtemp<-2


### DONE
ascitestemp<-rep(0, 3344)

### DONE
discancrtemp<-rep(0, 3344)
CMCaddComplication<-cbind(CMCaddComplication, discancrtemp)
temp<-filter(CMCaddComplication, DISCANCR=="Yes")$CASEID
CMCaddComplication[CMCaddComplication$CASEID %in% temp,]$discancrtemp<-6

### DONE
CCIagetemp<-round((CMCaddComplication$AGE-40)/10)
CCIagetemp[CCIagetemp<0]=0
CMCaddComplication<-cbind(CMCaddComplication, CCIagetemp)

### DONE
CCI<-CMCaddComplication$cdmitemp+ CMCaddComplication$hxchftemp+ CMCaddComplication$hxpvdtemp+ CMCaddComplication$cnscvatemp+ CMCaddComplication$hxcopdtemp+ CMCaddComplication$diabetestemp+ CMCaddComplication$renafailtemp+ CMCaddComplication$discancrtemp+CCIagetemp
CMCaddComplication<-cbind(CMCaddComplication, CCI)


CCItb<-table(CMCaddComplication[, c("CCI", "Complication")])
CCIchisq<-chisq.test(CCItb, simulate.p.value = TRUE)
```
